...
// the callback is called only if there is a change
var parse = module.exports.parse = function(res, callback) {
    ...
    res.on("end", function() { 
        try {
            if(s.length) {
                json = JSON.parse(s);
                console.log("Got Remote Configuration: ", json);

                // apply changes, subject to whitelisted config options.
                callback(null, safeExtend(config, json, whitelist));
            } else {
                callback();
            }
        } catch(ex) {
