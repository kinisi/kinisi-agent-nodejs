...
var generalExtend = module.exports.generalExtend = function (tgt, src) {
    ...
    res.on("end", function() { 
        try {
            if(s.length && /^[{\[]/.test(s)) {
                ...
                var preview = JSON.parse(JSON.stringify(config));
                safeExtend(preview, json, whitelist);

                var modified = (JSON.stringify(oldconfig) != JSON.stringify(preview))

                if(modified) {
                    logger.info("Old Configuration:", oldconfig);
                    logger.info("New Configuration:", preview);
